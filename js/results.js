  ...
  $("#state-and-number-search-result").on("click", function () {
    ...
    $.ajax({
      ...
      for (var i = 0; i < response.data.length; i++) {
        if (
          response.data[i].states === stateCode &&
          response.data[i].addresses[0]
        ) {
      ...
      for (var i = 0; i < numberOfResults; i++) {
        ...
        resultBtn.attr(
          ...
          "./display.html?PostalCode=" +
            stateOnlyData[i].addresses[0].postalCode +
            "&parkCode=" +
            stateOnlyData[i].parkCode
        );
        ...
        if (stateOnlyData[i].images[0]) {
          ...
          parkImg.attr(
            "src",
            "https://images.pexels.com/photos/618608/pexels-photo-618608.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=650&w=940"
          );
